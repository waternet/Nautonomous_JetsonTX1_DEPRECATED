...
#include "../include/nautonomous_cooperation_bluetooth_node/BluetoothException.h"
#include "../include/nautonomous_cooperation_bluetooth_node/BTSerialPortBinding.h"
...
void BTSerialPortBinding::Connect()
{
	...
	ROS_INFO("allocated socket: %i", status);

	if (status != 0)
		throw BluetoothException("Cannot connect");

	ROS_INFO("connedted to server");
}
