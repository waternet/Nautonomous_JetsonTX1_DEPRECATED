...
void rcCallback(const mavros_msgs::RCIn::ConstPtr& msg) {
	...
	//Only act if the takeover switch is active
	if(takeover){

		float propulsion = 0.0;
		// if the propulsion is active, then check the channel bounds and set the propulsion value
		...
		float steering = 0.0;
		// if the steering is active, then check the channel bounds and set the propulsion value
		...
		float conveyor = 0.0;
		// if the conveyor is active, then check the channel bounds and set the propulsion value
